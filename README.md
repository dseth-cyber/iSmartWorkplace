...
1.แตก `iSmartWorkplace-3.6.0-Windows-x64.zip` เครื่องแม่ Server แล้วดับเบิ้ลคลิก iSmartWorkplace-Server.exe ไม่ต้องปิด เปิดทิ้งไว้ แล้วดับเบิ้ลคลิกใช้เลย หรือติดตั้งตัว Setup iSmartWorkplace.exe (สำหรับแชทในองกรณ์)
2.เครื่องลูกแล้วคัดลอก `iSmartWorkplace.exe` ไปไว้ในตำแหน่งถาวร ไม่ต้องติดตั้ง Python ดับเบิ้ลคลิกใช้เลย หรือติดตั้งตัว Setup (สำหรับแชทในองกรณ์)
3.ฟังชั่น Backup ดับเบิ้ลคลิกใช้เลย หรือติดตั้งตัว Setup ไม่ต้องทำอย่างอื่น
3. กฎเก่าจะปรากฏในเมนู **โฟลเดอร์และตารางเวลา** เป็นงานลบอย่างเดียว ใช้เกณฑ์วันที่และสถานะเปิด/ปิดเดิม แนะนำพักตารางเวลาในรุ่นเก่าก่อนอัปเดตเพื่อมีเวลาตรวจสอบ
4. เริ่มด้วยทดลองสแกน ตรวจรายการใน **ประวัติการทำงาน → ดูรายละเอียด / CSV** แล้วจึงเปลี่ยนเป็นทำงานจริงและเปิดตารางเวลา
5. ปิดหน้าต่างเพื่อย่อไป Tray เปิด/พักตารางเวลาและออกจากโปรแกรมได้จากเมนู Tray หากเครื่องไม่มี Tray จะย่อหน้าต่างแทน
6. **ตั้งค่าและวิธีใช้ → เปิดโปรแกรมใน Tray เมื่อเข้าสู่ระบบ Windows** ใช้กับบัญชีผู้ใช้ปัจจุบัน หากย้าย EXE ไปที่ใหม่ให้ปิดแล้วเปิดตัวเลือกนี้อีกครั้ง
